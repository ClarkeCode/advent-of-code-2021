...
bool processCommand(FILE* fp, Position* pos) {
	...
	if (strncmp(buff, "forward", 7) == 0) { pos->horizontal += (size_t)atoi(buff+7); }
	if (strncmp(buff, "up",      2) == 0) { pos->depth      -= (size_t)atoi(buff+2); }
	if (strncmp(buff, "down",    4) == 0) { pos->depth      += (size_t)atoi(buff+4); }
